...
def create_weekday_figure(dfs, figures_dir):
    ...
    for i, hour in enumerate(hours):
        plt.text(i, hour + max(hours) * 0.05, f"{hour:.1f}", 
    ...
    plt.tight_layout()
    plt.savefig(f"{figures_dir}/weekday_playtime.pdf", bbox_inches="tight", format="pdf")
    plt.close()
...
def create_hourly_figure(dfs, figures_dir):
    ...
    for i, hour in enumerate(hours_y):
        plt.text(i, hour + max(hours_y) * 0.05, f"{hour:.1f}", 
                ha="center", va="bottom", fontsize=16)
